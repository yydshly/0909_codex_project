...
`web/assets/architecture.svg` 为按上述源码职责绘制的详细架构说明图。箭头表示简化主链；直接创作、助手、不同供应商和任务恢复并不共享完全相同的执行路径。网页同时提供逐层文字说明。

网页中的业务对象关系图、分批调度图和六步执行示意用于解释同一套实现。六步示意假定通用 Kie 路径中的图片与视频任务成功，不执行真实生成。场景、价值和扩展方向为研究判断，未宣称上游已实现所建议的能力。
