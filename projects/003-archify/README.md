...
> Archify 将用户或 Agent 编写的结构化 JSON 规格转成可交互、可导出的系统设计图，支持五类技术图与架构模型差异；适用于系统讲解、设计评审、团队交接和重构沟通。核心价值是把系统知识交付为可校验、可复现、便于追踪关系的图形文档；可进一步接入代码分析、扩展 UML / ER 等图型，并融入持续更新的工程文档流程。

## 库的能力与价值

| 维度 | 说明 |
| --- | --- |
| 核心能力 | 原生绘制架构图、工作流、时序图、数据流和生命周期图；比较两个架构模型，展示变更前、差异和变更后。提供节点搜索、上下游追踪、有向路径查询、讲解视图及多格式导出。 |
| 技术原理 | 外部作者或 Agent 整理事实并编写 JSON → 规格校验 → 专用布局与渲染 → 产物检查 → 交付独立 HTML。架构图还可关联固定 Git 提交中的源码位置。 |
| 使用场景 | 新成员理解系统组成与请求链路；设计评审讨论职责和数据流；重构前后比较模型；团队交接、技术分享和项目文档配图。 |
| 核心价值 | 用不同视角表达复杂系统，降低解释与沟通成本；将图规格、源码引用与检查记录一起保留，使绘图过程可重复、交付结果可复查，并支持交互阅读和分享。 |
| 可扩展方向 | 在输入侧接入 AST、依赖分析、接口与数据库结构、运行追踪等事实来源；在模型与渲染侧补充 UML 类图、ER 图、部署图等；在流程侧集成 CI、版本化文档和模型更新。以上为扩展建议，需另行实现。 |
| 能力边界 | 当前库不内置通用的代码关系自动提取能力，也不自动从 Git 差异理解架构变化。代码事实和前后模型需要用户、外部 Agent 或分析工具提供；五类图不等于完整 UML 支持。 |
...
| 整理日期 | 2026-09-09 |
| 发布状态 | 已发布至 GitHub Pages，见上方公开网页入口 |
